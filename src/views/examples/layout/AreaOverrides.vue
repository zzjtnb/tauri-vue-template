...
const examples = [
  { to: '/examples/layout/regions/whole-header', title: '整区 Header/Footer 组件', description: 'header/footer 通过 { component } 接管整区，sidebar 子区仍可单独覆盖。' },
  { to: '/examples/layout/regions/components', title: '四个整区显式组件配置', description: 'header/sidebar/content/footer 全部使用 { component }，验证统一声明路径。' },
  { to: '/examples/layout/regions/all-parts', title: 'all 批量子区', description: 'header/sidebar/content/footer 都通过 all 批量填充子区，并用显式子区覆盖局部。' },
  { to: '/examples/layout/regions/no-settings', title: '关闭默认设置入口', description: 'layout.settings: false 关闭默认设置入口，页面仍保留其它 header 区域。' },
]
