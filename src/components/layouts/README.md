...
本文是使用者文档，按第三方库 README 编写。源码内直接使用时，可以把示例里的 `vue-app-layouts` 替换为 `@/components/layouts`。
...
```bash
pnpm add vue-app-layouts
```

如果使用源码目录内的组件，不需要安装包，直接从项目别名导入：

```ts
import { AppLayout, LayoutRuntime, LayoutSettings, LayoutSettingsTrigger } from '@/components/layouts'
import type { Layout, LayoutRouteModule, LayoutRouteRecord } from '@/components/layouts'
```

发布为第三方包时，按包名导入：

```ts
import { AppLayout, LayoutRuntime, LayoutSettings, LayoutSettingsTrigger } from 'vue-app-layouts'
import type { Layout, LayoutRouteModule, LayoutRouteRecord } from 'vue-app-layouts'
```
...
```ts
import { AppLayout } from 'vue-app-layouts'
...
```ts
import { LayoutRuntime } from 'vue-app-layouts'
...
```ts
import type { Layout } from 'vue-app-layouts'
...
```vue
<script setup lang="ts">
import { LayoutRuntime, LayoutSettings } from 'vue-app-layouts'
...
import type { LayoutRouteModule } from 'vue-app-layouts'
