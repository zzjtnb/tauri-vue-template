 ...
 * 布局主题色名称。
 *
 * 这些值对应 `src/assets/styles/theme/tokens` 下的主题 token class。
 ...
 */
interface MenuItem extends Required<Pick<DisplayMeta, 'title'>>, Pick<DisplayMeta, 'icon'>, MenuMeta {
  // 菜单唯一标识，优先路由名，兜底路径。
  name: string
  // 已归一化排序值。
  order: number
...
interface Theme {
  ...
  Mode: ThemeMode
  // 主题色名称，对应 src/assets/styles/theme/tokens 下的主题类。
  Color: ThemeColor
  // 主题完整值，作为 preferences.theme 的持久化形状。
  Value: ThemeValue
}
