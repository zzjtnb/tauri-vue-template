...
import { NativeSelect, NativeSelectOption } from '@/components/shadcn-vue/ui/native-select'
...
<template>
  <DefineMonthTemplate v-slot="{ date }">
    <div class="**:data-[slot=native-select-icon]:right-1">
      <div class="relative">
        <div class="text-sm pl-2 flex h-full pointer-events-none items-center inset-0 absolute">
          {{ formatter.custom(toDate(date), { month: 'short' }) }}
        </div>
        <NativeSelect
          class="text-xs text-transparent pl-2 pr-6 h-8 relative"
  ...
  >
    <CalendarHeader class="pt-0">
      <nav class="flex gap-1 items-center inset-x-0 top-0 justify-between absolute">
        <CalendarPrevButton>
