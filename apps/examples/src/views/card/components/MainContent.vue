...
import { Button, Input, Label, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Tabs, TabsContent, TabsList, TabsTrigger } from '@tauri-vue-template/ui'
        ...
        <div v-if="generatedCards.length > 0" class="mt-6 space-y-4">
          ...
          <div class="gap-4 grid lg:grid-cols-3 md:grid-cols-2">
            ...
            >
              ...
              <button
                type="button"
                class="group text-xl tracking-wider font-mono font-semibold px-2 py-2 text-left rounded bg-black/20 flex cursor-pointer items-center justify-between"
                title="点击复制卡号"
                :aria-label="`复制卡号 ${formatPan(card.number)}`"
                @click="copyToClipboard(card.number)"
              >
                <span class="text-white mix-blend-difference">{{ formatPan(card.number) }}</span>
                <i class="i-lucide-copy ml-2 opacity-0 flex-shrink-0 h-4 w-4 transition-opacity group-hover:opacity-100" aria-hidden="true" />
              </button>
              ...
              <div class="flex items-end justify-between">
                <button
                  ...
                  title="点击复制姓名"
                  :aria-label="`复制持卡人姓名 ${card.name}`"
                  @click="copyToClipboard(card.name)"
                >
                  <span class="text-xs text-white/80 mb-1 block mix-blend-difference">
                    持卡人
                  </span>
                  <span class="text-white font-semibold block mix-blend-difference">
                    {{ card.name }}
                  </span>
                </button>
                ...
                <div class="ml-4 flex gap-4">
                  <button
                    ...
                    title="点击复制有效期"
                    :aria-label="`复制有效期 ${card.expiry}`"
                    @click="copyToClipboard(card.expiry)"
                  >
                    <span class="text-xs text-white/80 mb-1 block mix-blend-difference">
                      有效期
                    </span>
                    <span class="text-white font-mono font-semibold block mix-blend-difference">
                      {{ card.expiry }}
                    </span>
                  </button>
                  <button
