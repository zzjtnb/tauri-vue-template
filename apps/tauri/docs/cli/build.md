...
- `src-tauri/tauri.conf.json` 当前配置示例应用的 `beforeBuildCommand`，执行 `pnpm tauri build` 会先构建示例应用前端产物。
- `src-tauri/tauri.conf.json` 当前配置 `frontendDist: "../../examples/dist"`，对应示例应用 Vite 输出目录。
- bundle/installer 类型和签名能力受执行平台影响；跨平台构建必须在对应系统或完整交叉编译环境里验证。
- 修改构建相关配置时，同时检查 `src-tauri/tauri.conf.json` 和对应平台接入指南。
