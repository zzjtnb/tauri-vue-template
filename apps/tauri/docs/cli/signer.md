...
| `-w, --write-keys <WRITE_KEYS>` | 将私钥写入指定文件 | `pnpm tauri signer generate -w ~/.tauri/examples.key` |
| `-f, --force` | 强制覆盖指定路径上的已有私钥 | `pnpm tauri signer generate -w ~/.tauri/examples.key -f --ci` |
| `--ci` | CI 中跳过交互提示 | `pnpm tauri signer generate -w ~/.tauri/examples.key -p '<password>' --ci` |
| `CI` | 通过环境变量跳过交互提示 | `CI=true pnpm tauri signer generate -w ~/.tauri/examples.key -p '<password>'` |
...
这里的文件通常是 Tauri updater 更新包文件。示例假设发布流程把更新包收集到 `release/apps/<app>/v<version>/<platform>/updater/`：
...
```bash
pnpm tauri signer sign ./release/apps/examples/v0.1.0/macos/updater/示例.app.tar.gz -f ~/.tauri/examples.key -p '<password>'
```
...
| 覆盖项 | 场景 | 命令 |
| --- | --- | --- |
| `<FILE>` | 指定要签名的 release 文件 | `pnpm tauri signer sign ./release/apps/examples/v0.1.0/macos/updater/示例.app.tar.gz -f ~/.tauri/examples.key` |
| `-k, --private-key <PRIVATE_KEY>` | 使用私钥字符串签名 | `pnpm tauri signer sign ./release/apps/examples/v0.1.0/macos/updater/示例.app.tar.gz -k '<private-key>' -p '<password>'` |
| `-v, --verbose...` | 签名时输出详细日志 | `pnpm tauri signer sign ./release/apps/examples/v0.1.0/macos/updater/示例.app.tar.gz -f ~/.tauri/examples.key -v` |
| `-f, --private-key-path <PRIVATE_KEY_PATH>` | 使用私钥文件签名 | `pnpm tauri signer sign ./release/apps/examples/v0.1.0/macos/updater/示例.app.tar.gz -f ~/.tauri/examples.key` |
| `-p, --password <PASSWORD>` | 使用私钥密码签名 | `pnpm tauri signer sign ./release/apps/examples/v0.1.0/macos/updater/示例.app.tar.gz -f ~/.tauri/examples.key -p '<password>'` |
| `TAURI_SIGNING_PRIVATE_KEY` | 通过环境变量注入私钥字符串 | `TAURI_SIGNING_PRIVATE_KEY='<private-key>' TAURI_SIGNING_PRIVATE_KEY_PASSWORD='<password>' pnpm tauri signer sign ./release/apps/examples/v0.1.0/macos/updater/示例.app.tar.gz` |
| `TAURI_SIGNING_PRIVATE_KEY_PATH` | 通过环境变量注入私钥文件路径 | `TAURI_SIGNING_PRIVATE_KEY_PATH=~/.tauri/examples.key pnpm tauri signer sign ./release/apps/examples/v0.1.0/macos/updater/示例.app.tar.gz` |
| `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` | 通过环境变量注入私钥密码 | `TAURI_SIGNING_PRIVATE_KEY_PATH=~/.tauri/examples.key TAURI_SIGNING_PRIVATE_KEY_PASSWORD='<password>' pnpm tauri signer sign ./release/apps/examples/v0.1.0/macos/updater/示例.app.tar.gz` |
...
- 本项目发布签名密码只通过 `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` 注入；不要把真实密码写进命令参数、脚本或日志。
