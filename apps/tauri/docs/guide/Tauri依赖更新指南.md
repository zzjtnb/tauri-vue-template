...
除非特别说明，下文命令默认在 **`apps/tauri` 目录内（方式 A）** 执行；在**仓库根（方式 B）** 请写成 `pnpm --dir apps/tauri <script>`。Android、iOS、鸿蒙、桌面脚本规则相同，详见 `apps/tauri/README.md` 与 `docs/guide/配置文件指南.md` 的「命令执行目录」。
...
示例 3：当前 `apps/tauri/package.json` 使用的 CLI 版本是：
...
本包不在 `apps/tauri/package.json` 中提供额外的依赖菜单。依赖维护按本文档手动执行，并用下面这些检查确认结果：
...
如果要更新 OpenHarmony 核心依赖，按 A3.1 执行 `cargo update`，再按 A4 和 A5 同步 npm 包。如果要更新单个插件，先改 `src-tauri/Cargo.toml` 中对应的 `tauri-plugin-*` 版本，再更新 `Cargo.lock`，最后把已安装的 `@tauri-apps/plugin-*` 同步到完全相同的版本。
...
移动端按目标平台补充验证（方式 A 简写；仓库根请加 `pnpm --dir apps/tauri` 前缀）：
...
如果要继续支持移动端，再验证（方式 A；根目录加 `pnpm --dir apps/tauri`）：
