...
`pnpm android:*` 在 `apps/tauri/package.json` 定义：在 `apps/tauri` 内直接执行，或在仓库根使用 `pnpm --dir apps/tauri android:*`（与 iOS / 鸿蒙相同，见 `apps/tauri/README.md`）。
...
Android dev 通过 Tauri 的 `beforeDevCommand` 启动默认示例应用 dev server，Vite 默认按 `development` mode 读取 `.env.development`，本机私有覆盖项可放到 `.env.local` 或 `.env.development.local`。
...
- `VITE_APP_TITLE` 默认是 `示例`，只影响页面标题或展示文案，不影响 Chrome inspect 是否能看到 WebView。
...
- 不要把带真实请求体的截图提交到仓库。
- 如果新增临时日志，只保留由环境变量控制、会脱敏、且集中封装的调试入口，不要把临时 `console` 或真实请求体散落在业务代码里。
