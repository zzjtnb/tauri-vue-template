...
本文定义本项目构建 Linux Tauri 应用的标准路径。Linux 使用现有 `src-tauri/` 工程；开发必须在 Linux 主机上进行，发布构建显式生成 DEB `.deb`。
...
6. 执行 `pnpm linux:build` 生成 DEB `.deb`。
7. 在目标发行版上安装或运行产物做验收。
...
```json
{
  "scripts": {
    "linux:dev": "tauri dev",
    "linux:build": "tauri build --target x86_64-unknown-linux-gnu --bundles deb"
  }
}
```
...
当前项目没有独立 Linux 覆盖文件。只有在 Linux 需要独立 bundle metadata、依赖声明或桌面文件配置时，才新增：

```text
src-tauri/tauri.conf.json 的 `bundle.linux`
```

当前 Linux 构建以基础配置和命令行 `--target`、`--bundles` 为准。
...
本项目当前 Linux 发布产物只保留 `.deb`。DEB 的仓库签名、包索引签名或外部分发签名由发行渠道处理。

建议在固定 CI 镜像中构建 Linux 产物，避免开发机系统库升级导致产物兼容性漂移。签名规则和 updater 签名边界见 `apps/tauri/docs/guide/签名与更新签名指南.md`。
...
通常是系统库、WebKitGTK 或发行版基线不匹配。发布构建应固定在兼容性更保守的 Linux 镜像中完成，并在目标发行版安装验收。
...
- [ ] `linux:build` 为 `tauri build --target x86_64-unknown-linux-gnu --bundles deb`。
- [ ] 插件权限已按跨平台和桌面端分开声明。
- [ ] 本地构建能生成 `.deb`。
- [ ] 发布产物在目标发行版完成安装和启动验收。
