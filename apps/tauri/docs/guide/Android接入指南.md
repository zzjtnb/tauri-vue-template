...
脚本在 `apps/tauri/package.json`。任选一种方式，**与 iOS / 鸿蒙相同**：

| | 写法 |
| --- | --- |
| **方式 A**（在 `apps/tauri` 内） | `cd apps/tauri` 后执行 `pnpm android:build` |
| **方式 B**（在仓库根） | `pnpm --dir apps/tauri android:build` |

下文步骤若只写 `pnpm android:*`，默认指**方式 A**；在根目录请改成方式 B。

标准初始化命令：

```bash
# 方式 A
cd apps/tauri && pnpm android:init

# 方式 B（仓库根）
pnpm --dir apps/tauri android:init
```

标准开发命令：

```bash
pnpm android:dev          # 方式 A
pnpm --dir apps/tauri android:dev   # 方式 B
```

标准构建命令：

```bash
pnpm android:build        # 方式 A
pnpm --dir apps/tauri android:build # 方式 B
```
...
4. **Android 专属 Tauri 配置写在主配置的 Android 字段中**。
   - Android 专属配置写在 `src-tauri/tauri.conf.json` 的 `bundle.android`。
...
```text
src-tauri/tauri.conf.json 的 `bundle.android`
```
...
Android dev 会读取 `build.devUrl`，并在需要时通过 `TAURI_DEV_HOST` 替换 host。

默认 `src-tauri/tauri.conf.json` 指向 **examples**，其 Vite 默认端口为 **3000**；**template** 应用默认 **3001**（`packages/cli/src/tauri/config.ts` 中 `devUrl` 与 `apps/template/vite.config.ts` 一致）。端口可通过 `VITE_APP_PORT` 覆盖。

HMR 在移动端调试时固定为 **1421**（与 examples / template 的 `vite.config.ts` 一致）。
...
使用 `pnpm --filter @tauri-vue-template/cli tauri:target -- dev --target template` 时，前端 dev server 必须在 **3001** 监听。
...
当前模板的 `src-tauri/tauri.conf.json` 的 `bundle.android` 只保留 `minSdkVersion` 和 `versionCode` 这类当前 schema 支持的字段。如果添加 `debugApplicationIdSuffix` 后出现 schema 报错，说明当前 Tauri CLI / `tauri-build` 版本不支持该字段；应先删除该字段，或升级到明确支持该字段的 Tauri 版本线后再使用。
...
- [ ] `src-tauri/tauri.conf.json` 的 `bundle.android` 存放 Android 专属配置，不新增独立 `tauri.android.conf.json`。
...
- `apps/tauri/docs/cli/android.md`
