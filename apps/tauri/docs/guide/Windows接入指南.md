...
本文定义本项目构建 Windows Tauri 应用的标准路径。Windows 使用现有 `src-tauri/` 工程；开发必须在 Windows 主机上进行，发布构建显式生成 NSIS `.exe` 安装包。
...
7. 执行 `pnpm windows:build` 生成 NSIS `.exe`。
8. 发布前配置代码签名证书。
...
```json
{
  "scripts": {
    ...
    "windows:build": "tauri build --target x86_64-pc-windows-msvc --bundles nsis"
  }
}
```
...
当前项目窗口和 bundle 信息由该文件统一维护。Windows 专属差异只在确实需要时新增：

```text
src-tauri/tauri.conf.json 的 `bundle.windows`
```

当前项目没有独立 Windows 覆盖文件，因此 Windows 构建以基础配置和命令行 `--target`、`--bundles` 为准。
...
正式发布前必须接入 Authenticode 签名证书和时间戳服务。签名规则和 updater 签名边界见 `apps/tauri/docs/guide/签名与更新签名指南.md`。
...
4. Tauri bundler 生成 NSIS `.exe` 安装包。
5. 如果配置签名，安装包进入代码签名阶段。
...
- [ ] `windows:build` 为 `tauri build --target x86_64-pc-windows-msvc --bundles nsis`。
- [ ] 插件权限已按跨平台和桌面端分开声明。
- [ ] 本地构建能生成 `.exe`。
