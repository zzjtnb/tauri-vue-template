...
目标：删除或移走 `src-tauri/` 后，尽量只用官方 Tauri CLI 重新生成项目。完整参数以 `docs/tauri/cli/` 为准，本文只保留本项目重建顺序。

## 已验证依据

- 已按 `docs/tauri/cli/{init,add,build,dev,icon,permission,capability}.md` 核对参数。
...
`fs`、`upload` 在模板原生能力页有真实示例入口；`autostart`、`updater`、`window-state` 是桌面能力。如果 `Cargo.toml` 按 target 隔离这些依赖，Rust 侧插件注册也必须使用一致的 target 条件，避免 Android/iOS 编译时引用不存在的 crate。

## 3. 补回项目 Rust 逻辑

当前 Rust 侧保留 `opened_urls` command 和 `RunEvent::Opened` 事件处理，作为文件打开/URL 打开场景的后端示例；前端模板默认不主动消费该事件。需要启用文件关联时，再在对应业务模块中调用 command 或监听 `opened` 事件。具体 Rust 分层、文件映射和不生成空架子的规则见 `docs/tauri/guide/structure.md`。
...
这里的 `build.beforeBuildCommand = "true"` 只用于首次生成 permission 元数据，避免触发前端 `pnpm build`；正式打包不要跳过前端构建。

如果不覆盖 `beforeBuildCommand`，这一步会先执行 `pnpm build`，前端类型检查或 Vite 构建失败会导致 Tauri 构建提前退出。
...
结合 `src/views/settings/native` 分区里的原生能力示例，只需要追加项目代码额外调用的权限：
...
移动端工程生成后再生成图标，确保 Android/iOS 相关资源也能按当前工程结构生成。

```bash
pnpm tauri icon public/logo.png
```

需要 Android 分层图标时，用 manifest 作为输入：

```bash
pnpm tauri icon path/to/icon-manifest.json
```
...
`dev` 会启动长运行进程；`build` 会执行 `beforeBuildCommand = "pnpm build"`。
