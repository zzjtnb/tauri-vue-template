...
`tauri icon` 用于从源图标生成各主要平台所需图标。它会写入 Tauri icons 目录，属于会改动资源文件的命令。输入可以是带透明通道的正方形 PNG、SVG，也可以是 manifest JSON；默认输入是 `./app-icon.png`。
...
- 只有 `default` manifest 属性必填。
- manifest JSON 中定义的所有文件路径都相对 manifest 文件本身。
- `bg_color` manifest 值如果设置，会覆盖 `--ios-color` 选项。
- `default` 通常指向通用源图标；`android_bg`、`android_fg`、`android_fg_scale`、`android_monochrome` 用于 Android 图标资源分层和缩放配置。

## 位置参数

| 参数 | 完整说明 |
| --- | --- |
| `[INPUT]` | 源图标路径或 manifest 文件路径。源图标必须是带透明通道的正方形 PNG 或 SVG；manifest 文件必须是 JSON。默认值：`./app-icon.png`。 |
...
| `--ios-color <IOS_COLOR>` | iOS 图标背景色，字符串格式遵循 W3C CSS Color Module Level 4。默认值：`#fff`。如果 manifest 中设置了 `bg_color`，manifest 的 `bg_color` 会覆盖该选项。 |
...
| `[INPUT]` | 指定 manifest JSON，且 manifest 内路径相对 manifest 文件 | `pnpm tauri icon ./assets/icon-manifest.json` |
...
| `--ios-color <IOS_COLOR>` | 指定 iOS 背景色；若 manifest 有 `bg_color` 会被覆盖 | `pnpm tauri icon ./assets/app-icon.png --ios-color '#ffffff'` |
