...
| `-c, --config <CONFIG>` | 使用 JSON 字符串临时覆盖配置 | `pnpm tauri ios init -c '{"identifier":"com.zzjtnb.zzjt"}' --ci` |
...
| 覆盖项 | 场景 | 命令 |
| --- | --- | --- |
| `[ARGS]...` | 给底层 iOS runner 传参 | `pnpm tauri ios build -t aarch64-sim --no-sign -- -v` |
| `-d, --debug` | debug 构建并输出详细日志 | `pnpm tauri ios build -d -t aarch64-sim --no-sign -v` |
| `-v, --verbose...` | 构建时输出详细日志 | `pnpm tauri ios build -t aarch64-sim --no-sign -v` |
| `-t, --target [<TARGETS>...]` | iOS 模拟器构建并跳过签名 | `pnpm tauri ios build -t aarch64-sim --no-sign` |
...
| `-c, --config <CONFIG>` | 使用 JSON 字符串临时跳过前端构建命令 | `pnpm tauri ios build -t aarch64-sim --no-sign -c '{"build":{"beforeBuildCommand":"true"}}'` |
...
| `-o, --open` | 构建后打开 Xcode | `pnpm tauri ios build -t aarch64-sim --no-sign -o` |
