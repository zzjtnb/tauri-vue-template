...
本文定义本项目构建 macOS Tauri 应用的标准路径。macOS 使用现有 `src-tauri/` 工程，不创建额外原生工程目录；开发、构建、签名和产物验收均围绕 Tauri 官方桌面流程维护。
...
3. **发布构建显式生成 macOS bundle**。
   ...
   - bundle 类型固定为 `app,dmg`。
4. **签名和公证属于发布门槛**。
   - 本地验证可以生成未签名产物。
   - 分发给用户的 `.app` / `.dmg` 必须完成 Developer ID 签名和 notarization。
...
6. 执行 `pnpm mac:build` 生成 `.app` 和 `.dmg`。
...
```json
{
  "scripts": {
    "mac:dev": "tauri dev",
    "mac:build": "tauri build --target universal-apple-darwin --bundles app,dmg"
  }
}
```
...
macOS 构建脚本通过 `--bundles app,dmg` 明确收敛最终产物，不依赖 `targets: "all"` 的平台展开结果。
...
```bash
spctl --assess --type execute --verbose path/to/争逐.app
```
...
4. Tauri bundler 生成 `.app`。
5. Tauri bundler 生成 `.dmg`。
6. 如果配置了签名和公证，进入签名、公证和 staple 阶段。
...
- [ ] `mac:dev` 为 `tauri dev`。
- [ ] `mac:build` 为 `tauri build --target universal-apple-darwin --bundles app,dmg`。
