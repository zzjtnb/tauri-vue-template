...
| 文件 | 职责 |
| --- | --- |
| `apps/*/uno.config.ts` | 各应用薄配置：从 `@tauri-vue-template/theme/unocss` 导入 `createUnoConfig` 并 `export default`。 |
| `packages/theme/src/unocss.ts` | UnoCSS 规则与 `theme.colors` 等映射的真实维护位置（把工具类映射到 CSS 变量）。 |
| `apps/*/src/main.ts` | 应用样式入口顺序控制；必须先导入 `uno.css`，再导入项目样式。 |
| `packages/theme/src/styles/theme/index.css` | 主题样式入口；导入基础变量和每个主题 token 文件。 |
| `packages/theme/src/styles/theme/variables.css` | 只放不适合由 Wind4 自动生成的项目基础变量，例如 `--radius`、`--un-default-border-color`。 |
| `packages/theme/src/styles/theme/tokens/*.css` | 每个主题的明暗 token 值，是颜色值的真实维护位置。 |
| `packages/theme/src/styles/index.css` | 全局样式入口；主题样式、基础样式、安全区、动画和第三方样式从这里接入。 |
...
`packages/theme/src/unocss.ts`（经应用 `uno.config.ts` 引用）使用：
...
因此 `apps/template/src/main.ts` 和 `apps/examples/src/main.ts` 必须保持这个顺序：
...
`uno.css` 先建立 layer 顺序；项目样式只挂到已有 layer。不要在 `packages/theme/src/styles/index.css` 里手写 `@layer fonts, properties, ...` 这类内部层级清单，避免和 UnoCSS 版本升级后的 resolved layers 漂移。

## UnoCSS 映射规则

`packages/theme/src/unocss.ts` 中的 `theme.colors` 只引用 CSS 变量，不维护具体颜色值：
...
具体颜色只改 `tokens/*.css`，不要在组件或 `unocss.ts` 里写死颜色值。
...
主题切换由布局运行时集中处理。`packages/ui/src/layouts/composables/useRuntime.ts` 会把主题状态同步到 `document.documentElement`：
...
默认主题状态定义在 `packages/ui/src/layouts/runtime/defaults.ts`：
...
字体变量由 `packages/theme/src/unocss.ts` 的 `theme.font` 和 `presetWebFonts` 统一维护：
...
1. 在 `packages/theme/src/styles/theme/tokens/` 下新增主题文件，例如 `green.css`。
2. 顶部注释写清楚亮色和暗色主色调。
...
不要在 `unocss.ts` 中新增具体颜色值；`unocss.ts` 只负责把 token 名映射到 CSS 变量。
...
2. 在 `packages/theme/src/unocss.ts` 的 `theme.colors` 中映射到 CSS 变量。
...
```bash
pnpm --dir apps/examples exec node --input-type=module -e "import config from './uno.config.ts'; import { createGenerator } from 'unocss'; const uno = await createGenerator(config); const result = await uno.generate('bg-primary text-primary-foreground bg-sidebar-primary text-sidebar-primary-foreground bg-warning text-warning-foreground', { preflights: false }); console.log([...result.matched].sort().join('|'));"
```
