...
const uiSourceGlob = fileURLToPath(new URL('../../ui/src/**/*.{vue,ts}', import.meta.url))

export interface ThemeUnoConfigOptions {
  // 宿主应用根目录；当 iconsDir 为相对路径时用于解析绝对路径。
  appRoot: string
  // 图标目录，可传绝对路径，或相对 appRoot 的路径。默认 src/assets/icons。
  iconsDir?: string
}

// 本地图标类名可能由配置或运行时代码间接引用，不能完全依赖内容扫描。
// 目录不存在时 safelist 为空；目录存在时明确保留 `i-svg:文件名`，避免生产构建时未出现的本地图标被 tree-shake 掉。
function generateSafeList(iconsDir: string) {
...
function resolveIconsDir(options?: ThemeUnoConfigOptions) {
  const iconsDir = options?.iconsDir ?? 'src/assets/icons'

  if (isAbsolute(iconsDir)) {
    return iconsDir
  }
  ...
  return resolve(options.appRoot, iconsDir)
}
...
export function createUnoConfig(options?: ThemeUnoConfigOptions) {
  const iconsDir = resolveIconsDir(options)
  ...
  // 样式缓存异常时可清理：rm -rf node_modules/.cache/unocss node_modules/.vite
    ...
    // pipeline 是 UnoCSS 的内容提取管线；这里使用运行时源码白名单，避免扫描 src-tauri、dist、backup、docs 和构建产物。
      ...
      // workspace UI 包通过源码方式被应用消费；其组件和布局中的 UnoCSS 类必须由宿主应用生成。
      // filesystem 直接扫描包源码，补足 Vite pipeline 只覆盖应用 src 的边界。
      filesystem: [
        uiSourceGlob,
      ],
    },
