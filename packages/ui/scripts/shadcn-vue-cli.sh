...
print_add_menu() {
  ...
  printf '%s\n' '1. 全量安装并覆盖 - 执行 add -a -y -o，安装全部组件并覆盖 src/ui'
...
rewrite_lucide_imports() {
  local target_dir="${1:-$GENERATED_ROOT}"
...
run_add_install_with_lucide_vue_next() {
  ...
  rewrite_lucide_imports "$GENERATED_ROOT"
}

run_lucide_import_rewrite() {
  local target_dir
  prompt_optional target_dir "扫描目录；留空默认 $GENERATED_ROOT: "
  target_dir="${target_dir:-$GENERATED_ROOT}"
